...
class NightLightsStore:
    def __init__(self, root: Path):
        ...
        self.ranking_path = find_first(root, "ranking_crecimiento.csv")
        self.metrics_path = find_first(root, "metricas_anuales_todas_las_zonas.csv")
        ...
        self.ranking = pd.read_csv(self.ranking_path, encoding="utf-8-sig", low_memory=False)
        self.metrics = (
            pd.read_csv(self.metrics_path, encoding="utf-8-sig", low_memory=False)
            if self.metrics_path
            else pd.DataFrame()
        )
        self.ranking["id_zona"] = self.ranking["id_zona"].astype(str)
        if not self.metrics.empty:
            self.metrics["id_zona"] = self.metrics["id_zona"].astype(str)
    ...
    def series(self, zone_id: str) -> list[dict[str, Any]]:
        if self.metrics.empty:
            return []
        rows = self.metrics[self.metrics["id_zona"].astype(str) == str(zone_id)].copy()
        if rows.empty:
            return []
